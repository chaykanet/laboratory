...
{   
    ...
    while ((p = strtok(NULL, " |")) != NULL && i < N + 1)
    {   
    ...
    if (sscanf(elemnts[3], "%d", &tmp) != 1 || tmp < 0)
        return ERR_IO;
    ...
    if (strcmp(elemnts[5], "new") == 0 || strcmp(elemnts[5], "New") == 0)
    {   
        newstate_t state;
        car->state_new = true;
    ...
    {   
        ...
        if (strcmp(elemnts[6], "-") == 0)
        {   
            if (sscanf(elemnts[7], "%hd",  &tmp_h) != 1 || tmp_h < 0)
            return ERR_IO;
            
            state.yearof = tmp_h;
        
            if (sscanf(elemnts[8], "%d", &tmp) != 1 || tmp < 0)
                return ERR_IO;
            state.mileage = tmp;
            ...
            if (sscanf(elemnts[9], "%hd", &tmp_h) != 1 || tmp_h < 0)
                return ERR_IO;
            state.owners = tmp_h;
            if (sscanf(elemnts[10], "%hd", &tmp_h) != 1 || tmp_h < 0)
                return ERR_IO;
            state.repairs = tmp_h;
